...
sub load_transitions{
    ...
    setup_variable( \my $transitions_directory, $parameters{Transition_Directory}, $self->{Transitions_Directory}, "transitions/" ) and do{
...
sub write_aggregates{
    ...
    setup_output( \my $output, $parameters{Output}, "aggregates.csv", \*STDOUT );
    ...
    if( !$transitions ){
        say $log "Error: Couldn't find transitions."; 
        return -1;
    }
    ...
    #$self->{Binaries};
    if( !$binaries ){
        say $log "Error: Couldn't find binaries." ;
        return -1;
    }
    
    #load_transitions() if !$self->{Transitions};
    #my $transitions = $self->{Transitions};
    #if( !$transitions ){
    ...
    #load_sequences() if !$self->{Binaries};
